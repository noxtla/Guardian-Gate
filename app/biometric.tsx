    ...
    const { userId, token, isBiometricEnabled } = useLocalSearchParams<{ userId: string; token: string; isBiometricEnabled: string }>();
    ...
    const triggerFlash = () => {
        // Anima la opacidad a 0.7 y luego de vuelta a 0 en una secuencia rápida
        flashOpacity.value = withSequence(
        withTiming(0.7, { duration: 50 }),
        withTiming(0, { duration: 250 })
        );
    };
    // --- FIN: SETUP DE LA ANIMACIÓN ---
    ...
    const handleVerification = async () => {
        ...
        // --- INICIO: DISPARAR ANIMACIÓN ---
        // Disparamos el flash inmediatamente para dar feedback instantáneo
        triggerFlash();
        // --- FIN: DISPARAR ANIMACIÓN ---
    ...
    return (
        ...
        {/* --- INICIO: VISTA PARA EL FLASH --- */}
        {/* Esta vista se superpone a todo y es controlada por la animación.
            pointerEvents="none" es crucial para que no bloquee los toques al botón. */}
        <Animated.View style={[styles.flashOverlay, animatedFlashStyle]} pointerEvents="none" />
        {/* --- FIN: VISTA PARA EL FLASH --- */}
    ...
    // --- INICIO: NUEVO ESTILO PARA EL FLASH ---
    flashOverlay: {
        ...StyleSheet.absoluteFillObject,
        backgroundColor: Colors.brand.white,
        zIndex: 1, // Asegura que esté por encima de la cámara
    },
    // --- FIN: NUEVO ESTILO PARA EL FLASH ---
    centerContent: {
        flex: 1,
    ...
    overlay: {
        ...
        zIndex: 2, // Asegura que la UI esté por encima del flash
    },
    header: {
        alignItems: 'center'
    },
